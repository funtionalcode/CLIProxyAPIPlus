...
func TestGeminiCLIInternalRouteRemainsRegistered(t *testing.T) {
	server := newTestServer(t)
...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	if !ok || len(serviceTiers) != 0 {
		t.Fatalf("expected compact gpt-5.5 service tiers, got %#v", gpt55["service_tiers"])
	}
	if custom == nil {
	...
	if got, _ := custom["base_instructions"].(string); got == "" || got == gpt55["base_instructions"] {
		t.Fatal("expected custom model to use compact model-specific base_instructions")
	}
	...
	if got := custom["apply_patch_tool_type"]; got != nil {
		t.Fatalf("expected custom apply_patch_tool_type to be null, got %#v", got)
	}
	if got := custom["upgrade"]; got != nil {
		t.Fatalf("expected custom upgrade to be null, got %#v", got)
	}
	if got := custom["availability_nux"]; got != nil {
		t.Fatalf("expected custom availability_nux to be null, got %#v", got)
	}
...
func TestUnifiedModelsIncludesContextMetadataForPlainModels(t *testing.T) {
	...
	claudeModel := findModelByID(claudeResp.Data, "claude-fable-5-dd-5.5-tpg")
	if claudeModel == nil {
		t.Fatal("expected encoded gpt-5.5 in Claude model list")
	}
...
func TestManagementUsageRequiresManagementAuthAndPopsArrayUpstream(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "test-management-key")
...
func TestModelsWithClientVersionReturnsCodexCatalogUpstream(t *testing.T) {
	modelRegistry := registry.GetGlobalRegistry()
	clientID := "test-client-version-catalog"
...
func TestServerResponsesStreamingRequestLogCapturesUpstreamSections(t *testing.T) {
	...
	mockLogger := internallogging.NewFileRequestLogger(true, logsDir, "", 10, 100)
