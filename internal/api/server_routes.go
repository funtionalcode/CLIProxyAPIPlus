...
func (s *Server) serveOAuthCallback(c *gin.Context, provider string) {
	...
	if cfg != nil && cfg.RemoteManagement.OAuthCallbackRedirect != "" {
		redirectURL, errParse := url.Parse(cfg.RemoteManagement.OAuthCallbackRedirect)
		if errParse == nil {
			q := redirectURL.Query()
...
func (s *Server) registerCustomOAuthRoutes(cfg *config.Config) {
	...
	for i := range cfg.CustomOAuth {
		providerConfig := cfg.CustomOAuth[i]
		name := strings.TrimSpace(providerConfig.Name)
		if name == "" {
			continue
		}
		callbackPath := strings.TrimSpace(providerConfig.CallbackPath)
		if callbackPath == "" {
		...
		log.Infof("custom OAuth callback registered: %s -> %s (auth-url: %s)", callbackPath, provider, providerConfig.AuthURL)
	}
}
