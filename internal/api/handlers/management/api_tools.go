...
func (h *Handler) APICall(c *gin.Context) {
	...
	resolveToken := func() error {
		if !tokenResolved {
			token, tokenErr = h.resolveTokenForAuthWithProxy(c.Request.Context(), auth, requestProxyURL)
			tokenResolved = true
		}
		if token == "" {
			if tokenErr != nil {
...
func (h *Handler) resolveTokenForAuth(ctx context.Context, auth *coreauth.Auth, proxyURLs ...string) (string, error) {
	requestProxyURL := ""
	if len(proxyURLs) > 0 {
		requestProxyURL = proxyURLs[0]
	}
	return h.resolveTokenForAuthWithProxy(ctx, auth, requestProxyURL)
}

func (h *Handler) resolveTokenForAuthWithProxy(ctx context.Context, auth *coreauth.Auth, requestProxyURL string) (string, error) {
	if auth == nil {
		return "", nil
	}
...
func (h *Handler) enrichCopilotTokenResponse(ctx context.Context, response apiCallResponse, auth *coreauth.Auth, originalURL, requestProxyURL string) apiCallResponse {
	...
	token, tokenErr := h.resolveTokenForAuthWithProxy(ctx, auth, requestProxyURL)
	if tokenErr != nil {
