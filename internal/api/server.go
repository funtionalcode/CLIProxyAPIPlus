...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	engine.Use(s.managerServiceProxyMiddleware())
...
func (s *Server) setupRoutes() {
	...
	s.registerManagerServiceProxyRoutes()
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/request-error-logs/:name/formatted", s.mgmt.DownloadFormattedRequestErrorLog)
		mgmt.GET("/request-error-logs/:name", s.mgmt.DownloadRequestErrorLog)
		mgmt.GET("/request-success-logs", s.mgmt.GetRequestSuccessLogs)
		mgmt.GET("/request-success-logs/:name/formatted", s.mgmt.DownloadFormattedRequestSuccessLog)
...
func (s *Server) unifiedModelsHandler(openaiHandler *openai.OpenAIAPIHandler, claudeHandler *claude.ClaudeCodeAPIHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if isAnthropicModelsRequest(c) || isClaudeCodeUserAgent(c.GetHeader("User-Agent")) {
	...
	for _, models := range bySection {
		for _, model := range models {
			...
			out = append(out, homeModelEntry{
				id:                  id,
				created:             homeModelInt64Value(model, "created", "created_at"),
				ownedBy:             ownedBy,
				displayName:         displayName,
