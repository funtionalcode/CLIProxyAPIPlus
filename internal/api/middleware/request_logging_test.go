...
func TestShouldCaptureRequestBody(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "logger enabled captures small known-size body",
			loggerEnabled: true,
			req: &http.Request{
		...
		{
			name:          "logger enabled spools over-limit body to disk",
			loggerEnabled: true,
			req: &http.Request{
		...
		{
			name:          "logger enabled spools unknown-size body to disk",
			loggerEnabled: true,
			req: &http.Request{
				Body:          io.NopCloser(strings.NewReader("x")),
				ContentLength: -1,
...
func TestDeferredRequestBodyCaptureDoesNotDrainUnreadBody(t *testing.T) {
	...
	logger := logging.NewFileRequestLogger(false, t.TempDir(), "", 10, 0)
...
func TestRequestLoggingMiddlewareCapturesLargeErrorRequestAndDeferredAPIRequest(t *testing.T) {
	...
	logsDir := t.TempDir()
	logger := logging.NewFileRequestLogger(false, logsDir, "", 10, 0)
...
func TestRequestLoggingMiddleware_StreamingResponsesUpstreamSections(t *testing.T) {
	...
	logsDir := t.TempDir()
	logger := logging.NewFileRequestLogger(true, logsDir, "", 10, 100)
