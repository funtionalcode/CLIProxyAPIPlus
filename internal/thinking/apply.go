...
func ApplyThinking(body []byte, model string, fromFormat string, toFormat string, providerKey string) ([]byte, error) {
	...
	if summaryConfig.Mode == SummaryUnspecified && !strings.EqualFold(strings.TrimSpace(fromFormat), strings.TrimSpace(toFormat)) {
		summaryConfig = ExtractSummaryConfig(body, fromFormat)
	}
...
func applyThinking(body, sourceBody []byte, model string, fromFormat string, toFormat string, providerKey string, resolvedModelInfo *registry.ModelInfo, modelInfoResolved bool, summaryConfig SummaryConfig) ([]byte, error) {
	...
	if summaryConfig.Mode == SummaryUnspecified && fromFormat == "gemini-cli" && isGeminiFamily(providerFormat) && !thinkingIsFullyDisabled(*validated) {
		summaryConfig = SummaryConfig{Mode: SummaryEnabled, Detail: "auto"}
	}
