...
func ExtractSummaryConfig(body []byte, format string) SummaryConfig {
	...
	case "gemini-cli":
		...
		if geminiCLIThinkingConfigIsActive(body) {
			return SummaryConfig{Mode: SummaryEnabled, Detail: "auto"}
		}
	case "interactions":
...
func geminiCLIThinkingConfigIsActive(body []byte) bool {
	config := gjson.GetBytes(body, "request.generationConfig.thinkingConfig")
	if !config.IsObject() {
		return false
	}
	if budget := gjson.GetBytes(body, "request.generationConfig.thinkingConfig.thinkingBudget"); budget.Exists() {
		return budget.Type != gjson.Number || budget.Int() != 0
	}
	if level := gjson.GetBytes(body, "request.generationConfig.thinkingConfig.thinkingLevel"); level.Type == gjson.String {
		levelValue := strings.TrimSpace(level.String())
		return levelValue != "" && !strings.EqualFold(levelValue, "none")
	}
	return false
}
