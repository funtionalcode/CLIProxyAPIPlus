		...
		for i := 0; i < len(normalized); i++ {
			...
			case "function_call":
				...
				if rawSignature := strings.TrimSpace(item.Get("_cpa_reasoning_signature").String()); rawSignature != "" {
					signature = openAIResponsesGeminiFunctionCallSignature(rawSignature)
				}
			...
			case "reasoning":
				thoughtText := responseReasoningSummaryText(item)
				...
				if strings.TrimSpace(rawSignature) != "" {
					functionSignature = openAIResponsesGeminiFunctionCallSignature(rawSignature)
				}
				...
				if useGeminiNativeReasoningLayout && i+1 < len(normalized) {
					...
					if visible, ok := openAIResponsesAssistantVisibleText(next); ok && canBindText {
						visibleText = visible
						i++
					} else if next.Get("type").String() == "function_call" && canBindFunction && strings.TrimSpace(next.Get("_cpa_reasoning_signature").String()) == "" && functionSignature != "" {
						contentItems = append(contentItems, buildOpenAIResponsesReasoningFunctionCallModelContent(thoughtText, next, functionSignature))
...
func openAIResponsesGeminiThoughtSignature(rawSignature string) string {
	signature, ok := sigcompat.CompatibleSignatureForProviderBlock(sigcompat.SignatureProviderGemini, rawSignature, sigcompat.SignatureBlockKindGeminiModelPart)
	if !ok {
		return ""
	}
	return signature
...
func openAIResponsesGeminiFunctionCallSignature(rawSignature string) string {
	return sigcompat.GeminiReplaySignatureOrBypass(rawSignature, sigcompat.SignatureBlockKindGeminiFunctionCall)
}
