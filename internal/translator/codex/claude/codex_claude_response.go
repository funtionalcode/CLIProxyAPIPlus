...
type ConvertCodexResponseToClaudeParams struct {
	...
	// reverseNameMap caches short-to-original tool name mapping built from the original request.
	reverseNameMap map[string]string
	// idMap caches shortened-to-original tool call ID mapping built from the original request.
...
func ConvertCodexResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	if !params.mapsInitialized {
		params.reverseNameMap = ir.BuildClaudeToolNameReverseMap(originalRequestRawJSON)
		params.idMap = ir.BuildClaudeToolCallIDMap(originalRequestRawJSON)
		params.mapsInitialized = true
	}
