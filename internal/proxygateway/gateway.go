...
func (g *Gateway) startLocked() error {
	...
	go func() {
		if errServe := g.httpServer.Serve(ln); errServe != nil && errServe != http.ErrServerClosed {
...
func (g *Gateway) TestProxy(ctx context.Context, proxyURL string) (int64, error) {
	...
	req, err := http.NewRequestWithContext(testCtx, http.MethodHead, "https://www.cloudflare.com/cdn-cgi/trace", nil)
	if err != nil {
		return 0, err
	}
