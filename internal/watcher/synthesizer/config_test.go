...
func TestConfigSynthesizer_GeminiKeys(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			geminiKeys: []config.GeminiKey{
				{APIKey: "test-key-123", Weight: &weightThree},
			},
			wantLen: 1,
			validate: func(t *testing.T, auths []*coreauth.Auth) {
...
func TestConfigSynthesizer_OpenAICompat_CarriesProviderAndKeyWeights(t *testing.T) {
		...
		Config: &config.Config{
			OpenAICompatibility: []config.OpenAICompatibility{
				...
				},
				{
					...
					Weight:  1,
					APIKeyEntries: []config.OpenAICompatibilityAPIKey{
						{APIKey: "provider-b-key", Weight: &keyWeightThree},
					},
				},
			},
		},
	...
	if gotWeights["openai-compatible-providera"] != "2" {
		t.Fatalf("providera weight = %q, want 2", gotWeights["openai-compatible-providera"])
	}
	if gotWeights["openai-compatible-providerb"] != "3" {
		t.Fatalf("providerb weight = %q, want 3", gotWeights["openai-compatible-providerb"])
	}
}
