...
func TestFileSynthesizer_Synthesize_MultiProjectGemini(t *testing.T) {
	...
	for i := 1; i < 4; i++ {
		...
		if virtual.Status != coreauth.StatusActive {
			t.Errorf("expected virtual %d to be active, got %s", i, virtual.Status)
		}
		if virtual.Attributes["gemini_virtual_parent"] != primary.ID {
			t.Errorf("expected virtual %d parent to be %s, got %s", i, primary.ID, virtual.Attributes["gemini_virtual_parent"])
		}
		if gotPriority := virtual.Attributes["priority"]; gotPriority != "10" {
...
func TestFileSynthesizer_Synthesize_PreservesGeminiProviderFile(t *testing.T) {
	tempDir := t.TempDir()
	...
	if len(auths) != 1 || auths[0].Provider != "gemini-cli" {
		t.Fatalf("expected Gemini CLI auth file to remain supported, got %#v", auths)
	}
}
...
func TestFileSynthesizer_Synthesize_PreservesGeminiOAuthFile(t *testing.T) {
	tempDir := t.TempDir()
	...
	if len(auths) != 4 {
		t.Fatalf("expected Gemini CLI source and three project auths, got %d auths", len(auths))
	}
}
