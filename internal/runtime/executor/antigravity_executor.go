...
func newAntigravityHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	// Request logging is a local extension implemented by NewProxyAwareHTTPClient.
	// Keep using that wrapper when enabled instead of bypassing it with the proxy
	// fast path below.
	requestLogEnabled := cfg != nil && cfg.RequestLog
	if proxyURL := antigravityProxyURL(ctx, cfg, auth); proxyURL != "" && !requestLogEnabled {
	...
	// Clone the client before changing its transport because the proxy helper may
	// return a process-wide cached client that is also used by other providers.
