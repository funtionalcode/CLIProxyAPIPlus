...
func (e *CodexExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	turnstate.RecordResponseHeader(httpResp.Header, e.cfg, authID, baseModel)
	...
	go func() {
		...
		legacyBootstrapBuffering := !buffering
		var bootstrapChunks []cliproxyexecutor.StreamChunk
		bootstrapReleased := !legacyBootstrapBuffering
		...
		for scanner.Scan() {
			...
			if transformed, ok := grokbuild.TransformKeepaliveSSELine(translatedLine, isGrokClient); ok {
				...
				bootstrapOnly = codexStreamBootstrapOnlyDataType(eventType)
				switch eventType {
				case "response.output_item.done":
					collectCodexOutputItemDone(data, outputItemsByIndex, &outputItemsFallback)
				case "response.completed", "response.incomplete", "response.done":
