...
func resetProxyAwareHTTPClientCacheForTest(t *testing.T) {
	t.Helper()

	httpClientCacheMutex.Lock()
	originalCache := httpClientCache
	httpClientCache = make(map[string]*http.Client)
	httpClientCacheMutex.Unlock()

	t.Cleanup(func() {
		httpClientCacheMutex.Lock()
		httpClientCache = originalCache
		httpClientCacheMutex.Unlock()
	})
}

func TestResolveAntigravityGroundingURLsResolvesVertexRedirects(t *testing.T) {
	resetProxyAwareHTTPClientCacheForTest(t)
