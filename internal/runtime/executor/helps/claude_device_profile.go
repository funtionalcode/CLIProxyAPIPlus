...
func claudeDeviceProfileObservationKey(auth *cliproxyauth.Auth, apiKey string) string {
...
func claudeDeviceProfileObservationCacheKeys(auth *cliproxyauth.Auth, apiKey string) []string {
	return []string{claudeDeviceProfileObservationKey(auth, apiKey)}
}
...
func ObserveClaudeDeviceProfileHighWater(auth *cliproxyauth.Auth, apiKey string, headers http.Header, cfg *config.Config) bool {
	candidate, ok := extractClaudeDeviceProfile(headers, cfg)
	if !ok {
		return false
	}
	baseline := defaultClaudeDeviceProfile(cfg)
	candidate = pinClaudeDeviceProfilePlatform(candidate, baseline)
	if !shouldUpgradeClaudeDeviceProfile(candidate, baseline) {
		return false
	}
	claudeDeviceProfileCacheMu.Lock()
	recordClaudeDeviceProfileObservation(claudeDeviceProfileObservationKey(auth, apiKey), candidate)
	claudeDeviceProfileCacheMu.Unlock()
	return true
}
