...
func TestCodexExecutorExecuteStreamDelaysBootstrapEventsBeforeUsageLimit(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("event: response.created\n"))
		_, _ = w.Write([]byte(`data: {"type":"response.created","response":{"id":"resp_1","model":"gpt-5.5"}}` + "\n\n"))
		_, _ = w.Write([]byte("event: error\n"))
		_, _ = w.Write([]byte(`data: {"type":"error","error":{"type":"usage_limit_reached","message":"The usage limit has been reached","plan_type":"free","resets_in_seconds":120}}` + "\n\n"))
	}))
	defer server.Close()
	...
	for chunk := range result.Chunks {
		...
		if len(chunk.Payload) > 0 {
			t.Fatalf("got payload before usage_limit_reached error: %q", string(chunk.Payload))
		}
	}
	if streamErr == nil {
		t.Fatal("missing stream terminal error")
	...
	retryable, ok := streamErr.(interface{ RetryAfter() *time.Duration })
	if !ok || retryable.RetryAfter() == nil {
		t.Fatalf("stream error %T does not expose retry-after: %v", streamErr, streamErr)
	}
