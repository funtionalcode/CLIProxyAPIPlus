...
func codexCloakingEnabled(cfg *config.Config, auth *cliproxyauth.Auth) bool {
	if cfg == nil || isCodexCloakingDisabled(cfg, auth) {
		return false
	}
...
func applyCodexHeadersFromSources(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config, ginHeaders http.Header) {
	...
	if !managedIdentity && ginHeaders != nil && ginHeaders.Get("X-Codex-Beta-Features") != "" {
	...
	misc.EnsureHeader(r.Header, ginHeaders, "X-Codex-Turn-State", "")
	turnstate.InjectRequestHeader(r.Header, cfg)

	if strings.Contains(r.Header.Get("User-Agent"), "Mac OS") && codexSessionHeaderValue(r.Header) == "" {
