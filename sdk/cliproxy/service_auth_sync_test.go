...
func TestEndToEndStatusPatch_RestoresModelsThroughServerPipeline(t *testing.T) {
	...
	// 3. Disabled credentials may expose catalog previews but cannot serve models.
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodGet, "/v0/management/auth-files/models?name="+fileName, nil)
	req.Header.Set("Authorization", "Bearer test-secret")
	handler.ServeHTTP(rec, req)
	...
	if err := json.Unmarshal(rec.Body.Bytes(), &resp); err != nil {
		t.Fatalf("invalid model response after disable: %v", err)
	}
	if models := reg.GetModelsForClient(authID); len(models) != 0 {
		t.Fatalf("disabled credential still has serving models: %#v", models)
	}
