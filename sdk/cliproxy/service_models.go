...
func buildConfiguredModelInfo(model modelEntry, ownedBy, modelType string, created int64, fallbackDisplayName string, userDefined bool) *ModelInfo {
	...
	info := buildConfigModelInfo(alias, name, ownedBy, modelType, created)
	info.DisplayName = displayName
	info.UserDefined = userDefined
	info.MetadataModelID = name
	if info.MetadataModelID == "" {
		info.MetadataModelID = alias
	}
...
func buildOpenAICompatibilityConfigModels(compat *config.OpenAICompatibility) []*ModelInfo {
	...
	for i := range compat.Models {
		...
		if model.Image {
			info.Thinking = nil
			info.SupportedInputModalities = nil
			info.SupportedOutputModalities = nil
		} else if model.Thinking != nil {
			info.Thinking = modelconfig.NormalizeThinkingSupport(model.Thinking)
		...
		if inputModalities := normalizeCompatConfigModalities(model.InputModalities); len(inputModalities) > 0 {
			info.SupportedInputModalities = inputModalities
		}
		if outputModalities := normalizeCompatConfigModalities(model.OutputModalities); len(outputModalities) > 0 {
			info.SupportedOutputModalities = outputModalities
		}
...
func (s *Service) fetchKiroModels(a *coreauth.Auth) []*ModelInfo {
	...
	kiroAuth := kiroauth.NewKiroAuth(s.cfg)
	if kiroAuth == nil {
	...
	apiModels, errModels := kiroAuth.ListAvailableModels(ctx, tokenData)
	if errModels != nil {
		log.Warnf("kiro: failed to fetch dynamic models: %v, using static models", errModels)
		return registry.GetKiroModels()
	}
	if len(apiModels) == 0 {
...
// normalizeKiroModelID normalizes a Kiro model ID.
...
func generateKiroAgenticVariants(models []*ModelInfo) []*ModelInfo {
	...
	for _, model := range models {
		...
		agentic := &ModelInfo{
			...
			Description:         model.Description + " - Optimized for coding agents (chunked writes)",
