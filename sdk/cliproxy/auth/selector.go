...
func (s *RoundRobinSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
	now := time.Now()
	available, err := getContextAvailableAuthsForSelector(ctx, s, auths, provider, model, now)
	if err != nil {
		return nil, err
	}
	...
	if len(parentOrder) > 1 {
		...
		if s.Weighted {
			...
			s.ensureRotationKey(innerKey, limit)
			picked := group[successorIndex(group, s.lastPicked[innerKey])]
			s.lastPicked[innerKey] = picked.ID
			return picked, nil
		}
	...
	if s.Weighted {
		...
		return pickSmoothWeightedAuth(available, state.current), nil
	}
...
// ensureCursorKey ensures the grouped parent cursor map has capacity for the given key.
...
func (s *WeightedRoundRobinSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
	available, errAvailable := getSelectorAvailableAuthsAcrossPriorities(ctx, positiveWeightAuths(auths), provider, model, time.Now())
	if errAvailable != nil {
...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	if primaryID == "" {
		fallbackAuths, errAvailable := getContextAvailableAuthsForSelector(ctx, s.fallback, availabilityCandidates, provider, model, now)
		if errAvailable != nil {
			return nil, errAvailable
		}
...
func getContextAvailableAuthsForSelector(ctx context.Context, selector Selector, auths []*Auth, provider, model string, now time.Time) ([]*Auth, error) {
	weighted := selectorWeighted(selector)
	if _, ok := selector.(*WeightedRoundRobinSelector); ok {
		weighted = true
	}
	return getSelectorAvailableAuthsWithPriorityMode(ctx, auths, provider, model, now, weighted)
}
