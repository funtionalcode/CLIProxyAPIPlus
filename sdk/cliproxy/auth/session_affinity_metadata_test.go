...
func TestManagerSessionAffinityMixedPoolNilMetadataPropagatesFailureCleanup(t *testing.T) {
	...
	// 2. A second Execute call with fresh nil Metadata for the same session must
	// reuse auth-2. The manager may keep this binding in either the selector cache
	// or the built-in scheduler cache, so assert the routing behavior rather than
