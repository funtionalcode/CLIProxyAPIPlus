	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				...
				m.recordExecutionResult(ctx, Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, RetryAfter: retryAfterFromError(chunk.Err), Error: rerr}, auth, ephemeralResult)
			}
			if !forward {
				return false
			}
