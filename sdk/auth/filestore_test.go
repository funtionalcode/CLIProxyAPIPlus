...
func TestFileTokenStoreListMapsGeminiOAuthToGeminiCLI(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(
		filepath.Join(dir, "gemini-user.json"),
		[]byte(`{"type":"gemini","email":"user@example.com","access_token":"access","refresh_token":"refresh"}`),
		0o600,
	); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	...
	if got := auths[0].Provider; got != "gemini-cli" {
		t.Fatalf("Provider = %q, want gemini-cli", got)
	}
}
