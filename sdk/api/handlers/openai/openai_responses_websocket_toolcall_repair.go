...
func isResponsesToolCallType(itemType string) bool {
	return common.IsToolCallType(itemType)
}

func isResponsesToolCallOutputType(itemType string) bool {
	return common.IsToolCallOutputType(itemType)
}
...
func (t *responsesWebsocketToolCacheTurn) recordRequest(payload []byte) {
	...
	items, _, ok := parseResponsesWebsocketInputItemsNoCopy(payload, input)
	if !ok {
		return
	}
	for _, item := range items {
		t.recordInputItem(item)
	}
}
...
func repairResponsesWebsocketToolCallsWithCachesMode(
	...
	return repairResponsesToolCallsWithCachesMode(outputCache, callCache, sessionKey, payload, record, turn, false)
}

func repairResponsesToolCallsWithCachesMode(
	outputCache, callCache *websocketToolOutputCache,
	sessionKey string,
	payload []byte,
	record bool,
	turn *responsesWebsocketToolCacheTurn,
	repairWithoutSession bool,
) []byte {
	if len(payload) == 0 {
		return payload
	}
	...
	sessionKey = strings.TrimSpace(sessionKey)
	repairEnabled := outputCache != nil && (sessionKey != "" || repairWithoutSession)
...
func repairOpenAIResponsesHTTPRequestToolCalls(req *http.Request, payload []byte) []byte {
	return repairOpenAIResponsesToolCallsWithCachesMode(
		defaultWebsocketToolOutputCache,
		defaultWebsocketToolCallCache,
...
func openAIResponsesHTTPRequestSessionKey(req *http.Request, payload []byte) string {
	if promptCacheKey := strings.TrimSpace(util.GetGJSONBytesNoCopy(payload, "prompt_cache_key").String()); promptCacheKey != "" {
...
func repairOpenAIResponsesToolCallsWithCaches(outputCache, callCache *websocketToolOutputCache, sessionKey string, payload []byte) []byte {
	return repairOpenAIResponsesToolCallsWithCachesMode(outputCache, callCache, sessionKey, payload, true)
}

func repairOpenAIResponsesToolCallsWithCachesMode(outputCache, callCache *websocketToolOutputCache, sessionKey string, payload []byte, record bool) []byte {
	return repairResponsesToolCallsWithCachesMode(outputCache, callCache, sessionKey, payload, record, nil, true)
}
